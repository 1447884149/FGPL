...
# basic configuration
method=POIV5 #FedProto FML FedGH

script_floder="model_heterogeneity/SFL_methods/"${method}
result_folder_name="communication_consumption_result"
...
train_model() {
  python main.py --cfg ${main_cfg} \--client_cfg ${client_cfg} \
    ...
    exp_name ${exp_name} \
    eval.freq ${freq} \
    show_detailed_communication_info True
}
...
exp_name="communication_consumption_"$method"_on_"$data"_"$client_num"_clients"
train_model
