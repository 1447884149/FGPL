...
2023.09.04
核心思想：
每个client获取其他所有client的本地原型，作为补充知识；
...
class POIV4_Trainer(NodeFullBatchTrainer):
    def __init__(self,
                 model,
                 data,
                 device,
                 ...
                 only_for_eval=False,
                 monitor=None):
        super(POIV4_Trainer, self).__init__(model, data, device, config,
                                            only_for_eval, monitor)

        self.task = config.MHFL.task
        self.num_classes = config.model.num_classes

        self.ctx.global_model = LabelPropagation(num_layers=2, alpha=0.5)  # TODO: 超参数写进config
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if len(ctx.local_protos_from_other) == 0:
            loss = loss1
            PL_reps= torch.zeros_like(reps,device=reps.device)
            PL_pred = pred_ensemble_adaptive = torch.zeros_like(pred, device=pred.device)

        else:
            choosed_id =ctx.cfg.poi.temp_idx
            class_protos = ctx.local_protos_from_other[choosed_id]
            class_protos =  torch.stack(list(class_protos.values()))

            proto_lable_init = initialize_prototype_label(batch['train_mask'], class_protos, reps_all.detach(),
                                                          batch.y)

            PL_reps = ctx.global_model(y=proto_lable_init, edge_index=batch.edge_index, train_mask=batch['train_mask'])
    ...
    def update(self, local_protos, strict=False):
        self.ctx.local_protos_from_other = local_protos
...
def call_my_trainer(trainer_type):
    if trainer_type == 'poiv4_trainer':
        trainer_builder = POIV4_Trainer
        return trainer_builder


register_trainer('poiv4_trainer', call_my_trainer)
