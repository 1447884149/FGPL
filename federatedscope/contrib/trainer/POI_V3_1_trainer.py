...
"""
2023.9.9
在poiv3的基础上，调整KL_loss
思考为什么加了kl_loss之后效果变差的原因

观察结果：
(1)：KD-Loss 改为MSE后效果很差
(2): KD-loss tempure 增大后效果有所提升；（温度变大可能导致KD loss的影响权重变小）
(3): KD不是很好调
(4): * ensamble 在cora数据集上效果非常好，但在citeseer上一般
"""


# Build your trainer here.
class POIV3_1_Trainer(NodeFullBatchTrainer):
    def __init__(self,
                 model,
                 data,
                 device,
                 ...
                 only_for_eval=False,
                 monitor=None):
        super(POIV3_1_Trainer, self).__init__(model, data, device, config,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        if len(ctx.global_protos) == 0:
            ...
            pred_ensemble_adaptive = (0.5 * pred + 0.5 * PL_pred)
        ...
        logger.info(
            ...
            f'\t loss1:{loss1} \t loos2:{loss2},\t KD_loss{KD_loss}\ttotal_loss:{loss}')
...
def call_my_trainer(trainer_type):
    if trainer_type == 'poiv3_1_trainer':
        trainer_builder = POIV3_1_Trainer
        return trainer_builder


register_trainer('poiv3_1_trainer', call_my_trainer)
