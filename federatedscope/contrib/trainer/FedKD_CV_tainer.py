...
class FedKD_CV_Trainer(GeneralTorchTrainer):
    def __init__(self,
                 model,
                 data,
                 device,
                 ...
                 only_for_eval=False,
                 monitor=None):
        super(FedKD_CV_Trainer, self).__init__(model, data, device, config,
    ...
    def _hook_on_batch_forward(self, ctx):
        ...
        local_pred, local_rep = ctx.model(x)
        global_pred, global_rep = ctx.global_model(x)
        ...
        KL_local = self.KL_Loss(self.LogSoftmax(local_pred), self.Softmax(global_pred.detach())) / (
                CE_local + CE_global)  # TODO:检查detach()操作是否会导致bug，计算图是否正确
...
def call_my_trainer(trainer_type):
    if trainer_type == 'fedkd_cv_trainer':
        trainer_builder = FedKD_CV_Trainer
        return trainer_builder


register_trainer('fedkd_cv_trainer', call_my_trainer)
