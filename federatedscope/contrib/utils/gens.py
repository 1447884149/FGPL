...
def saliency_mixup(x, sampling_src_idx, sampling_dst_idx, lam, global_protos, sampling_list):
    """
    ...
    for i in range(7):
        ...
        if (len(global_protos)>0):
            one_class_mixed_node = lam[:int(sampling_list[i])] * new_src + (0.5-lam[:int(sampling_list[i])]) * new_dst +0.5*global_protos[i]
        else:
