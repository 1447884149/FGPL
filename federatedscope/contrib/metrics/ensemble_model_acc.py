...
import torch
import numpy as np
METRIC_NAME = 'ensemble_model_acc'

def ensemble_model_acc(ctx,y_true, **kwargs):
    labels = y_true
    y_prob = torch.Tensor(torch.cat(ctx.ensemble_ys_prob)).to(ctx.device)
...
def call_my_metric(types):
    if METRIC_NAME in types:
        ...
        metric_builder = ensemble_model_acc
